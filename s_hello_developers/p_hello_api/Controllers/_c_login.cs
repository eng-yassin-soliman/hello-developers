...
namespace p_hello_api.Controllers
{
    ...
    public class _c_login : Controller
    {
        ...
        public string f_signup_(_c_member p_mem_)
        {
            ...
            byte[] l_byt_ = Encoding.UTF8.GetBytes(p_mem_.s_pas_);
            // Get the password bytes
            byte[] l_hsh_ = _c_security.f_hash_(l_byt_);
            // Hash the password
            string l_pas_ = BitConverter.ToString(l_hsh_);
            // hex encoded hash
        ...
        public string f_saymyname_()
        {
            ...
            if( string.IsNullOrEmpty(l_hex_))
            // No cookies
            {
            ...
            byte[] l_cph_ = (from i_hex_ in l_hxs_ select byte.Parse(i_hex_, NumberStyles.HexNumber)).ToArray();
            // Decrypt the byte array
            byte[] l_out_ = _c_security.f_decrypt_(l_cph_);
            string l_str_ = Encoding.UTF8.GetString(l_out_);
            if(string.IsNullOrEmpty(l_str_))
            // Corrupt cookies
            {
                return "خطأ في تعريف بيانات الارتباط";
            }
            ...
            n_final_:l_dal_.f_close_(false);
            return l_rtr_;
        }
